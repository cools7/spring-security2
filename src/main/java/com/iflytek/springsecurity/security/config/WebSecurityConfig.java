...
import com.iflytek.springsecurity.security.filter.JWTAuthenticationFilter;
import com.iflytek.springsecurity.security.filter.ValidateCodeFilter;
import com.iflytek.springsecurity.security.handler.*;
 ...
 *
 *
 *  被@PreAuthorize("hasRole('super')")注释的 或者  资源在数据库里分配对应角色的 被权限管理
 *  没被注释 或者 资源在数据库里也没有分配对应角色   这种资源不受权限限制  所有用户都能访问
 *
 *
 * @Author: zule
 * @Date: 2019/5/6
...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        ...
        http.csrf().disable()
                ...
                //登录页面路径
                .loginPage("/api/index")
                //登录url 此登录url 和Controller 无关系
                .loginProcessingUrl(securityLoginPath)
                //登录成功跳转路径
                .successForwardUrl("/api/index")
                //登录失败跳转路径
                .failureUrl("/api/index")
                .permitAll()
